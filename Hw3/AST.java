class Class {
	...
	void indent(IndentOutput out, int n) {
		...
		out.indent(n+1, "Methods");
		Methods.indent(out, n+2, methods, null);
	...
	void java(JavaOutput out, int n, String parent, Args parentArgs, Methods parentMethods) {
		if(isAbstract == true)
			out.print(0, "abstract ");
		out.printName(n, name, parent);
		Args.displayFields(out, n+2, args, parentArgs);
		Args.displayCons(out, n+2, args, parentArgs, name);
		Methods.displayFunc(out, n+2, methods);
		if(methods == null)
			Methods.displayFunc(out, n+2, parentMethods);
		out.print(n+2, "// add additional methods and fields here ... \n");
		out.print(n, "}\n\n");
		java(out, n, subclasses, name, args, methods);
	}
	public static void java(JavaOutput out, int n, Class[] classes, String parent, Args parentArgs, Methods methods) {
		for (int i=0; i<classes.length; i++){
			classes[i].java(out, n, parent, parentArgs, methods);
		}
	}
...
class Args {
	...
	public static void displayCons(JavaOutput out, int n, Args args, Args parentArgs, String name) {
		out.print(n, "// declare default constructor for this class\n");
		out.print(n, name + "(");
...
class Methods {
	...
	public static void indent(IndentOutput out, int n, Methods methods, Methods end) {
		if(methods != end) {
			indent(out, n, methods.before, end);
			methods.last.indentName(out, n);
			methods.last.indentRet(out, n+1);
			out.indent(n+1, "Params");
			methods.last.indentParam(out, n+2);
		}
	}

	public static void displayFunc(JavaOutput out, int n, Methods methods) {
		if(methods != null) {
			displayFunc(out, n, methods.before);
			methods.last.displayName(out, n);
			out.print(0, "(");
			methods.last.displayParams(out, n);
			out.print(0, ") {\n");
			out.print(n+2, "// edit this method body\n");
			out.print(n, "}\n\n");
		}
	}
}
...
class Method {
	...
	public void indentName(IndentOutput out, int n) {
		StringBuffer buf = new StringBuffer();
		buf.append("Method ");
		buf.append(name);
		out.indent(n, buf.toString());
	}
	public void indentRet(IndentOutput out, int n) {
		StringBuffer buf = new StringBuffer();
		buf.append("ReturnType=" + ret.toString());
		out.indent(n, buf.toString());
	}
	public void indentParam(IndentOutput out, int n) {
		for(int i=0; i<params.length; i++) {
			out.indent(n, params[i].toString());
		}
	}

	public void displayName(JavaOutput out, int n) {
		StringBuffer buf = new StringBuffer();
	...
	public void displayParams(JavaOutput out, int n) {
		if(params.length > 0)
			out.print(0, params[0].toString());
		for(int i=1; i< params.length; i++) {
			out.print(0, ", ");
			out.print(0, params[i].toString());
		}
	}
}
