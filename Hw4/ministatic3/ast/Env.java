...
public class Env {
	...
	//flag = 1 is used. Otherwise it's not
	private int flag;
    ...
    public static Env lookup(Id id, Env env) {
		...
		//System.out.print(id.coordString() + " ");
		//System.out.print(env.getId() + "flag: ");
		//System.out.println(env.getFlag());
                return env;
            }
        }
        return null;
    ...
    public static void Display(Env env) {
	if(env == null)
		return;
	Display(env.rest);
	if(env.getFlag() == 0) {
        	System.out.print(env.id.coordString() + " ");
        	System.out.print(env.getId() + "\tflag: ");
        	System.out.println(env.getFlag());
	}
   }
     ...
     */
   /* public void insert(Env v) {
	if(uses == null) {
		uses = new IdList(v.getId(), null);
	} else {
		IdList temp = new IdList(v.getId(), uses);
		uses = temp;
	}
    }
