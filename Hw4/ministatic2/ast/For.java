...
public class For extends PosStmt {
	...
	env = first.analyze(scoping, env);
	env = step.analyze(scoping, env);
	env = test.analyze(scoping, env);
	env = body.analyze(scoping, env);
	return env;
	...
	test.superAnalyze(typing, Type.BOOLEAN);
	step.superAnalyze(typing, Type.INT);
	body.analyze(typing);
	...
	initialized = first.analyze(init, initialized);
	VarSet fid = test.analyze(init, initialized);
	VarSet dis = step.analyze(init, initialized);
	body.analyze(init, initialized);
	return VarSet.union(VarSet.intersect(VarSet.trim(fid, initialized),
				VarSet.trim(dis, initialized)), initialized);
